...
class DouyinLive:
    ...
    def start_ws(self):
        ...
        if not isinstance(room_info, dict):
            raise RuntimeError(f"Unexpected live room info payload: {room_info}")
        ...
        ws_user_unique_id = str(room_info.get("ws_user_unique_id") or user_id)
        ...
        (params
         ...
         .add_param('user_unique_id', ws_user_unique_id)
         ...
         .add_param('signature', generate_signature(room_id, ws_user_unique_id))
         )
        wss_url = f"wss://webcast5-ws-web-lf.douyin.com/webcast/im/push/v2/?{urlencode(params.get())}"
        ws_cookie_dict = dict(self.auth_.cookie or {})
        ws_cookie_dict["ttwid"] = ttwid
        ws_cookie = "; ".join([f"{k}={v}" for k, v in ws_cookie_dict.items()])
